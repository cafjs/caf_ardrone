...
var ArDroneConfig = exports.ArDroneConfig = function(setupConfig) {
    var udpCon = setupConfig.udpControl;
    // it can take a second to process this, ignoring other requests...
    udpCon.raw('CONFIG_IDS', SESSION_ID, PROFILE_ID, APPLICATION_ID);
    udpCon.raw('CONFIG', '"custom:session_id"', SESSION_ID);
    udpCon.raw('CONFIG_IDS', SESSION_ID, PROFILE_ID, APPLICATION_ID);
    udpCon.raw('CONFIG', '"custom:profile_id"', PROFILE_ID);
    udpCon.raw('CONFIG_IDS', SESSION_ID, PROFILE_ID, APPLICATION_ID);
    udpCon.raw('CONFIG', '"custom:application_id"', APPLICATION_ID);
    this.config = {};
    this.pending = {};
};
...
var merge = function(pending, newConfig) {
    newConfig = newConfig || {};
...
var prefixMultiConfig = function(udpCon) {
    udpCon.raw('CONFIG_IDS', SESSION_ID, PROFILE_ID, APPLICATION_ID);
};

ArDroneConfig.prototype.update = function(client, newConfig) {
    var udpCon = client.udpCon;
    newConfig = merge(this.pending, newConfig);
    Object.keys(newConfig)
        .forEach(function(key) {
                     var obj = newConfig[key];
                     if (obj !== this.config[key]) {
                         if (obj !== null) {
                             this.config[key] = obj;
                             prefixMultiConfig(udpCon);
                             udpCon.config(key, obj);
                         } else {
                             // to enable a retry
...
ArDroneConfig.prototype.altitud_max = function(value) {
    this.pending['control:altitud_max'] = value;
    return this;
};
...
ArDroneConfig.prototype.bitrate = function(value) {
    // multi-config only
    this.pending['video:bitrate'] = value;
    this.pending['video:bitrate_control_mode'] = 2; // VBC_MANUAL
    return this;
};
